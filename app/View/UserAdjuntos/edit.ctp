...
<div class="col-md-2 well2">
    ...
    <ul class="nav">
        ...
        <li><?php echo $this->Form->postLink('Eliminar', array('action' => 'delete', $this->Form->value('UserAdjunto.id')),array('class'=>'btn glyphicon glyphicon-remove'), __('Realmente desea eliminar el registro con Id %s?', $this->Form->value('UserAdjunto.id'))); ?></li>
        <li><?php echo $this->Html->link('Listado', array('action' => 'index'),array('class'=>'btn glyphicon glyphicon-list')); ?></li>
            </ul>
</div>
<div class="col-md-10">
    <h3>
        <?php echo __('Editar User Adjunto'); ?>    </h3>
    <?php echo $this->Form->create('UserAdjunto',array('onsubmit'=>'return checkSubmit();','role'=>'form','class'=>'form-horizontal')); ?>
	<fieldset>
	...
	<?php
		...
		echo $this->Form->input('tipo_adjunto_id',array('between'=>'<div class="col-sm-4">','after'=>'</div>','class'=>'form-control','label'=>array('class'=>'control-label col-sm-2'),'div'=>array('class'=>'form-group')));
